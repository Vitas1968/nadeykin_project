...
LLM выключена по умолчанию через `TENDER_LLM_ENABLED=false`.
Рабочая LLM demo-конфигурация описана в `tender-assistant-skill/.env.example` и `tender-assistant-skill/docs/selective_shadow_architecture.md`.

LLM-result будет храниться как `rule["llm_verdict"]`.

`rule["llm_verdict"]` является advisory metadata.
